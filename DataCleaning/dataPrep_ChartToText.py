...
for split_name, split_path in SPLITS.items():
    ...
    # -----------------------------------------------------
    # CREATE base dirs: images/, data/, captions/
    # -----------------------------------------------------
    for ct in valid_types:
        base = OUT / split_name / ct
        (base / "images").mkdir(parents=True, exist_ok=True)
        (base / "data").mkdir(parents=True, exist_ok=True)
        (base / "captions").mkdir(parents=True, exist_ok=True)
    ...
    for _, row in df_split.iterrows():
        ...
        # Determine actual source location (two_col vs multi_col)
        if row["raw_type"] == "two_col":
            img_path = ROOT / "imgs" / f"{cid}.png"
            data_txt = ROOT / "data" / f"{cid}.txt"
            data_csv = ROOT / "data" / f"{cid}.csv"
            caption_txt = ROOT / "captions" / f"{cid}.txt"
            caption_csv = ROOT / "captions" / f"{cid}.csv"

        else:  # multi_col
            img_path = ROOT / "multiColumn" / "imgs" / f"{cid}.png"
            data_txt = ROOT / "multiColumn" / "data" / f"{cid}.txt"
            data_csv = ROOT / "multiColumn" / "data" / f"{cid}.csv"
            caption_txt = ROOT / "multiColumn" / "captions" / f"{cid}.txt"
            caption_csv = ROOT / "multiColumn" / "captions" / f"{cid}.csv"
        ...
        # Output folders
        out_base = OUT / split_name / chart_type
        out_img = out_base / "images" / f"{cid}.png"
        out_data_txt = out_base / "data" / f"{cid}.txt"
        out_data_csv = out_base / "data" / f"{cid}.csv"
        out_caption_txt = out_base / "captions" / f"{cid}.txt"
        out_caption_csv = out_base / "captions" / f"{cid}.csv"
        ...
        # COPY DATA FILE
        if data_txt.exists():
            shutil.copy(str(data_txt), str(out_data_txt))
        elif data_csv.exists():
            shutil.copy(str(data_csv), str(out_data_csv))

        # -----------------------------
        # COPY CAPTION FILE
        if caption_txt.exists():
            shutil.copy(str(caption_txt), str(out_caption_txt))
        elif caption_csv.exists():
            shutil.copy(str(caption_csv), str(out_caption_csv))
        else:
            # No caption found → skip silently
            pass
